...
const plan = ref({ start_date: 0, end_date: 0 });
    ...
    axios
      ...
      .then((user) => {
        if (user) {
          ...
          let idx = participantChange.value.removed.indexOf(user);
          if (idx < 0) {
            participantChange.value.added.push(user);
          } else {
            participantChange.value.removed.splice(idx, 1);
          }
        } else {
          alert("존재하지 않는 유저입니다.");
        }
...
const removeParticipant = (user) => {
  ...
  idx = participantChange.value.added.indexOf(user);
  if (idx < 0) {
    participantChange.value.removed.push(user);
  } else {
    participantChange.value.added.splice(idx, 1);
  }
};
...
const update = () => {
  ...
  if (sessionStorage.getItem("attractions"))
    attractions.value = JSON.parse(sessionStorage.getItem("attractions"));
};
...
const saveTmpAcc = (accs) => {
  ...
  sessionStorage.setItem(
    "accommodations",
    JSON.stringify(accommodations.value ? accommodations.value : [])
  );
};

const saveTmpAttr = (day, attrs) => {
  if (day && attrs) attractions.value[day - 1] = attrs;
  sessionStorage.setItem("attractions", JSON.stringify(attractions.value ? attractions.value : []));
};
...
watch(
  ...
  (newVal, oldVal) => {
    if (newVal < oldVal) {
      // for (let i = 1; i <= oldVal - newVal; i++) {
      //   if (attractions.value[attractions.value.length - i]) {
    ...
    attractions.value.length = newVal;
  }
);
...
<template>
  ...
  <div class="container relative lg:w-[50vw] z-0">
    <div class="flex flex-col m-5">
      ...
      <div class="my-5">
        <RouterLink :to="{ name: 'planCreateAccommodation' }" @click="saveTmpPlan" push>
      ...
      <div class="my-5">
        ...
        <div v-for="day in planDayRange" class="ms-5 mt-5">
          <RouterLink
            :to="{ name: 'planCreateAttraction', query: { day } }"
            @click="saveTmpPlan"
            push
          >
            <input
            ...
            />
            <div class="bg-green-100 mt-2 py-1">
              <div v-if="!attractions || !attractions.length" class="text-gray-400 p-1">
                일정이 없습니다
              </div>
